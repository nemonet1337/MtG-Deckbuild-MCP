import { DeckBuildRequest, DeckBuildResult, DeckCard, FORMAT_LABELS, isSingletonFormat, MtgColor, MtgFormat, ScryfallCard } from "../types/mtg.js";
...
function budgetQuery(budget: DeckBuildRequest["budget"]): string {
  if (budget === "budget") return "usd<=3";
  if (budget === "mid") return "usd<=15";
  return "";
}
...
export class DeckBuilderService {
  ...
  async recommendCards(request: DeckBuildRequest): Promise<Record<string, ScryfallCard[]>> {
    const base = [formatLegalityQuery(request.format), colorIdentityQuery(request.colors), budgetQuery(request.budget), "-is:funny"].filter(Boolean).join(" ");
    ...
    return { ramp, draw, interaction, synergy, winConditions, lands, sideboard };
  }
