...
[Scryfall Languages](https://scryfall.com/docs/api/languages) に従い、カードの入出力を多言語で扱えます。

- **入力**: 英語 Oracle 名に加え、印刷名(例: `太陽の指輪`、`Contresort`)でもカード解決できます。`get_card_details`・`analyze_deck`・`build_deck` の commander/mustInclude・`suggest_budget_alternatives` が対象です
- **出力**: 任意の `lang` パラメータ(Scryfall コード)で当該言語の印刷を優先し、`printedName` / `printedTypeLine` / `printedText` / `lang` と、その印刷の画像 URL を返します
- **`name` フィールド**: 常に英語 Oracle 名です(ルール上の正式名)。デッキリスト文字列(`decklist`)も英語 Oracle 名で正規化します
- **対応コード**: `en` `es` `fr` `de` `it` `pt` `ja` `ko` `ru` `zhs` `zht` `he` `la` `grc` `ar` `sa` `ph` `qya` `dw`
- **未指定時**: 従来どおり英語 Oracle 印刷
- **希望言語の印刷が無い場合**: エラーにせず、解決できた印刷を返し `printedName` 等は省略します
- **制約**: Scryfall の `/cards/autocomplete` は英語名のみのため、本サーバーでも多言語オートコンプリートは提供しません。Oracle テキストは常に英語です(ゲームルール準拠)
...
返却例の要点: `name` は `Sol Ring`、`printedName` は `太陽の指輪`、画像は日本語印刷の `image_uris`。
...
`search_cards` / `recommend_cards` / `build_deck` / `analyze_deck` / `suggest_budget_alternatives` / `deck_wizard`(state.lang) も同じ `lang` を受け付けます。
...
- **多言語**: [Languages](https://scryfall.com/docs/api/languages) と [検索構文の lang:](https://scryfall.com/docs/syntax) に従い、印刷フィールド(`printed_name` 等)と `lang:` フィルタを利用します。Oracle 名は英語のまま保持し、ゲーム上の同一性判定(ban リスト等)も英語名基準です
